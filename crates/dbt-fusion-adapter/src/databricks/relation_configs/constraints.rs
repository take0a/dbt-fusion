...
mod tests {
    ...
    fn create_mock_non_null_constraints_table() -> AgateTable {
        let column_names = vec!["col_name".to_string()];
        let column_types = vec!["string".to_string()];
        let rows = vec![
            Value::from(vec!["id".to_string()]),
            Value::from(vec!["name".to_string()]),
        ];
    ...
    fn create_mock_check_constraints_table() -> AgateTable {
        ...
        let rows = vec![
            Value::from(vec![
                "delta.constraints.valid_id".to_string(),
                "id > 0".to_string(),
            ]),
            Value::from(vec![
                "delta.constraints.name_length".to_string(),
                "length(name) > 2".to_string(),
            ]),
            Value::from(vec![
    ...
    fn create_mock_primary_key_constraints_table() -> AgateTable {
        let column_names = vec!["constraint_name".to_string(), "column_name".to_string()];
        let column_types = vec!["string".to_string(), "string".to_string()];
        let rows = vec![
            Value::from(vec!["pk_users".to_string(), "id".to_string()]),
            Value::from(vec!["pk_composite".to_string(), "org_id".to_string()]),
            Value::from(vec!["pk_composite".to_string(), "user_id".to_string()]),
        ];
        AgateTable::from_rows(column_names, column_types, rows)
    }
    ...
    fn create_mock_foreign_key_constraints_table() -> AgateTable {
        let column_names = vec![
            ...
            "parent_catalog_name".to_string(),
            "parent_schema_name".to_string(),
            "parent_table_name".to_string(),
            "parent_column_name".to_string(),
        ];
