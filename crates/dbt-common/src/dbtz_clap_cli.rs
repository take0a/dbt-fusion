...
use crate::node_selector::{parse_model_specifiers, IndirectSelection};
...
static ABOUT: LazyLock<String> = LazyLock::new(|| {
    format!(
        "dbtz {}: A fast, source-available dbt parser",
...
static AFTER_HELP: LazyLock<String> = LazyLock::new(|| {
    format!(
        "{}",
        BOLD.apply_to(
            "Use `dbtz <COMMAND> --help` to learn more about the options for each command."
        )
    )
});
