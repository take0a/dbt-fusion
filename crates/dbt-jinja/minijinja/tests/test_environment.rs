...
fn test_expression() {
    ...
    ctx.insert("bar", 23);
    assert_eq!(
        expr.eval(&ctx, Rc::new(DefaultRenderingEventListener))
            .unwrap(),
        Value::from(65)
    );
}
...
fn test_globals() {
    ...
    assert_eq!(
        tmpl.render((), Rc::new(DefaultRenderingEventListener))
            .unwrap()
            .0,
        "42"
    );
...
fn test_iter() {
    ...
    let renders = env
        ...
        .map(|(name, tmpl)| {
            (
                name,
                tmpl.render(&ctx, Rc::new(DefaultRenderingEventListener))
                    .unwrap()
                    .0,
            )
        })
