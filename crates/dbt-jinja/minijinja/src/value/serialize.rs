...
use super::mutable_map::MutableMap;
use super::mutable_map_with_capacity;
...
impl Serializer for ValueSerializer {
    ...
    fn serialize_seq(self, len: Option<usize>) -> Result<Self::SerializeSeq, InvalidValue> {
        Ok(SerializeSeq {
            elements: MutableVec::with_capacity(untrusted_size_hint(len.unwrap_or(0))),
        })
    }
    ...
    fn serialize_map(self, len: Option<usize>) -> Result<Self::SerializeMap, InvalidValue> {
        Ok(SerializeMap {
            entries: mutable_map_with_capacity(len.unwrap_or(0)),
            key: None,
        })
    }
